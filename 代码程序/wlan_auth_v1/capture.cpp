...
__u32 listen_events(const nl80211_state& state, const int cmds_count, const __u32* cmds)
{
...
public:
    ...
    byte* public_parameter;
    time_t& system_time;

    parameter_from_management_frame(const char* ssid, byte* param, time_t& time) : specific_ssid(ssid), public_parameter(param), system_time(time) { }
};

static int capture_public_parameters(struct nl_msg* msg, void* arg)
{
    ...
    if (bss[NL80211_BSS_INFORMATION_ELEMENTS]) {
        ...
        if (is_specific_ssid) { // 如果SSID是指定SSID
            ie = o_ie;
            ielen = o_ielen;
            while (ielen >= 2 && ielen >= ie[1]) {
                if (ie[0] == 221) { // 搜索供应商自定义参数
                    ...
                    if (len < 3)
                        continue;
                    if (data[0] == 0x11 && data[1] == 0x22 && data[2] == 0x33 && len == 11) {
                        argument->system_time = 0;
                        for (int i = 3; i < 11; i++) {
                            argument->system_time <<= 8;
                            argument->system_time |= data[i];
                        }
                    }
                    else if (data[0] == 0x33 && data[1] == 0x22 && data[2] == 0x11 && len == 35) {
                        for (int i = 3; i < 35; i++)
                            argument->public_parameter[i - 3] = data[i];
                    }
                }
...
int capture_public_parameter_and_system_time_from_80211_management_frame(const char* ifname, const char* specific_ssid, byte public_parameter[], time_t& system_time)
{
    ...
    parameter_from_management_frame argument(specific_ssid, public_parameter, system_time);
    ...
    register_handler(capture_public_parameters, &argument);
