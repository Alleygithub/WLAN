	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	...
	sn = wpa_bss_get_vendor_ie(bss, 0x54386201); // serial number
	if (sn && sn[1] == 12) {
		ret = os_snprintf(pos, end - pos, "\t");
		if (os_snprintf_error(end - pos, ret))
			return -1;
		pos += ret;
		for (i = 0; i < 14 ; i++) {
			ret = os_snprintf(pos, end - pos, "%02x", sn[i]);
			if (os_snprintf_error(end - pos, ret))
				return -1;
			pos += ret;
		}
	...
	phy_param = wpa_bss_get_vendor_ie(bss, 0x54386202); // physical parameter
	if (phy_param && phy_param[1] == 36) {
		ret = os_snprintf(pos, end - pos, "\t");
		if (os_snprintf_error(end - pos, ret))
			return -1;
		pos += ret;
		...
		for (i = 0; i < 38 ; i++) {
			ret = os_snprintf(pos, end - pos, "%02x", phy_param[i]);
			if (os_snprintf_error(end - pos, ret))
				return -1;
			pos += ret;
		}
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	ret = os_snprintf(pos, end - pos, "ssid / serial number / physical parameter\n");
	if (os_snprintf_error(end - pos, ret))
		return pos - buf;
	pos += ret;
