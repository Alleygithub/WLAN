...
inline unsigned int rotate_left(unsigned int x, unsigned int y)
{
    return ((x << (y & (32 - 1))) | (x >> (32 - (y & (32 - 1)))));
}

static void message_expand(unsigned int W[132], const unsigned int BB[16]) // 消息扩展
{
    for (int j = 0; j < 16; j++)
        W[j] = BB[j];
    for (int j = 16; j < 68; j++) {
...
static void compression_function(unsigned int V[8], const unsigned char message[64]) // 压缩函数
{
    unsigned int A = V[0];
    unsigned int B = V[1];
    unsigned int C = V[2];
...
static size_t padding(unsigned char padding_message[128], const unsigned char message[], size_t length) // 填充
{
    for (int i = 0; i < 128; i++)
        padding_message[i] = 0;
    ...
    for (int i = 1; i <= 8; i++) {
        padding_message[padding_message_length - i] = (unsigned char)bit_length;
        bit_length >>= 8;
    }
...
static void sm3_hash(const unsigned char message[], size_t length, unsigned char digest[])
{
    unsigned int V[8] = {
        0x7380166F,
        0x4914B2B9,
    ...
    unsigned char padding_message[128];
    ...
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 4; j++)
            digest[i * 4 + j] = (unsigned char)(V[i] >> (24 - j * 8));
...
/* 和wpa_supplicant建立连接.
 * 返回0表示连接成功，返回-1表示执行错误。
 */
static int connect_to_wpa_supplicant()
{
	if (os_program_init())
		return -1;
...
/* 获取初始口令。
 * 返回0表示获取成功，返回-1表示执行失败。
 */
static int get_initial_psk(const char *psk_path, unsigned long long *sn, unsigned char *psk)
{
	FILE *fp = NULL;
	char c = '\0';
	int j = 0;

	if ((fp = fopen(psk_path, "rt")) == NULL)
		return -1;
	fscanf(fp, "%llu", sn);
	fgetc(fp);
	for (j = 0; j < 32; j++) {
...
/* 获取AP当前口令的序列号，以及物理认证参数（如果有的话）。
 * 返回非负表示找到指定SSID，返回1表示解析出当前口令的序列号，返回2表示同时解析出物理认证参数，返回-1表示执行错误。
 */
static int get_physical_parameter(const char *ssid, size_t ssid_len, unsigned char *ap_sn, unsigned char *phy_param)
{
	...
	if (wpa_ctrl_request(ctrl_conn, cmd, strlen(cmd), buf, &len, wpa_cli_msg_cb) < 0)
		return -1;
	buf[len] = '\0';
	if (strncmp(buf, "ssid / serial number / physical parameter\n", 6) != 0)
		return -1;

	while (buf[i++] != '\n');
	while (buf[i] != '\0') {
		if (strncmp(buf + i, ssid, ssid_len) == 0) {
			i += ssid_len;
			if (buf[i] == '\t') {
				i += 13;
				for (j = 0; j < 8; j++) {
 ...
 * 返回网络ID号，返回-1表示执行错误。
 */
static int get_network_id(const char *ssid)
{
	return 0;
}
...
/* 计算口令。
*/
static void calculate_psk(const unsigned char *psk, const unsigned char *phy_param, unsigned char *tmp_psk)
{
	int i = 0;
	unsigned char tmp[32] = { 0 };

	for (i = 0; i < 32; i++) {
		tmp[i] = psk[i] ^ phy_param[i];
	}
	sm3_hash(tmp, 32, tmp_psk);
	return;
}

/* 设置预共享密钥。
 * 返回0表示设置成功，返回-1表示执行错误。
 */
static int set_psk(int net_id, const unsigned char *psk)
{
	char cmd[128] = { '\0' };
	char str_psk[65] = { '\0' };
	int i = 0;
	...
	for (i = 0; i < 32; i++)
		sprintf(str_psk + i * 2, "%02x", psk[i]);
	if (snprintf(cmd, sizeof(cmd) - 1, "SET_NETWORK %d psk %s", net_id, str_psk) >= sizeof(cmd))
		return -1;
	if (wpa_ctrl_request(ctrl_conn, cmd, strlen(cmd), buf, &len, wpa_cli_msg_cb) < 0)
		return -1;
	buf[len] = '\0';
...
/* 查看接入状态。
 * 返回0表示已连接，返回1表示未连接，返回2表示连接中，返回-1表示执行错误。
 */
static int status()
{
	char cmd[] = "STATUS";
	...
	if (strncmp(buf, "wpa_state=COMPLETED", 19) == 0)
		return 0;
	else if (strncmp(buf, "wpa_state=DISCONNECTED", 22) == 0 || strncmp(buf, "wpa_state=INACTIVE", 18) == 0)
		return 1;
	else if (strncmp(buf, "wpa_state=", 10) == 0)
		return 2;
	else
		return -2;
}
...
/* 接入WLAN。
 * 返回0表示成功接入，返回-1表示未成功接入，返回-2表示执行错误。
 */
static int join_in(int net_id)
{
	char cmd[64] = { '\0' };
	char buf[4096] = { '\0' };
	size_t len = sizeof(buf) - 1;
	...
	len = sizeof(buf) - 1;
	if (snprintf(cmd, sizeof(cmd) - 1, "ENABLE_NETWORK %d", net_id) >= sizeof(cmd))
		return -1;
	if (wpa_ctrl_request(ctrl_conn, cmd, strlen(cmd), buf, &len, wpa_cli_msg_cb) < 0)
		return -1;
	buf[len] = '\0';
	...
	while ((net_stat = status()) != 0 && time(NULL) - start < 5);
	if (net_stat == 0)
		return 0;
	else if (net_stat > 0)
		return -1;
...
/* 尝试接入WLAN。
 * 返回0表示成功接入，返回-1表示无法接入，返回-2表示执行错误。
 */
static int try_join_in(int net_id)
{
	char cmd1[64] = { '\0' };
	char cmd2[64] = { '\0' };
	char buf[4096] = { '\0' };
	...
	if (snprintf(cmd1, sizeof(cmd1) - 1, "DISABLE_NETWORK %d", net_id) >= sizeof(cmd1))
		return -1;
	if (wpa_ctrl_request(ctrl_conn, cmd1, strlen(cmd1), buf, &len, wpa_cli_msg_cb) < 0)
		return -1;
	buf[len] = '\0';
	if (strncmp(buf, "OK", 2) != 0)
		return -1;

	while ((net_stat = status()) != 1);

	len = sizeof(buf) - 1;
	if (snprintf(cmd2, sizeof(cmd2) - 1, "SELECT_NETWORK %d", net_id) >= sizeof(cmd2))
		return -1;
	if (wpa_ctrl_request(ctrl_conn, cmd2, strlen(cmd2), buf, &len, wpa_cli_msg_cb) < 0)
		return -1;
	buf[len] = '\0';
	if (strncmp(buf, "OK", 2) != 0)
		return -1;
...
/* 更新口令。
 * 返回0表示更新成功，返回-1表示执行错误。
 */
static int update_initial_psk(const char *psk_path, const unsigned char *tmp_psk, unsigned char *psk, unsigned long long *sn)
{
	FILE *fp = NULL;
	int i = 0;

	(*sn)++;
	memcpy(psk, tmp_psk, 32);

	if ((fp = fopen(psk_path, "wt")) == NULL)
		return -1;
	fprintf(fp, "%lld ", *sn);
	for(i = 0; i < 32; i++)
		fprintf(fp, "%02x", psk[i]);
	fclose(fp);
...
int main()
{
	char ssid[] = "wlan_auth"; // 指定SSID。
	size_t ssid_len = strlen(ssid); // 指定SSID长度。
	char psk_path[] = "/home/cyk/Documents/psk.txt"; // 口令文件路径。
	int net_id = -1; // 网络ID号。
	int ret = -1; // 中间函数返回值。
	int i = 0;
	clock_t start = 0, end = 0;

	unsigned long long sn; // 当前口令的序列号。
	unsigned long long ap_sn; // AP当前口令的序列号。
	unsigned char psk[32]; // 当前口令。
	unsigned char phy_param[32]; // 物理认证参数。
	unsigned char tmp_psk[32]; // 临时新口令。

	if ((ret = get_initial_psk(psk_path, &sn, psk)) < 0) // 获取当前口令及序列号。
		return ret;

	if ((ret = connect_to_wpa_supplicant()) < 0) // 和wpa_supplicant建立连接。
		return ret;

	if ((ret = get_network_id(ssid)) < 0) // 获取指定SSID的网络ID号。
		goto out;
	net_id = ret;
	...
	for (i = 0; i < 32; i++)
		printf("%02x", psk[i]);
	printf("\n");
	if ((ret = set_psk(net_id, psk)) < 0) // 设置初始口令。
		goto out;

	while (1) {
		start = clock();

		if ((ret = get_physical_parameter(ssid, ssid_len, (unsigned char*)&ap_sn, phy_param)) < 0) // 获取AP口令的序列号和物理认证参数（如果有的话）。
			goto out;

		if (ret == 0) // 未发现指定SSID的WLAN信号。
			continue;
		...
		else if (sn + 1 == ap_sn && ret == 2) { // 需要且能够更新口令。
			ret = -1;
			printf("在时间 %ld 获得物理认证参数：", time(NULL));
			for (i = 0; i < 32; i++)
				printf("%02x", phy_param[i]);
			printf("\n");
			calculate_psk(psk, phy_param, tmp_psk); // 计算新口令。
			printf("在时间 %ld 得到新口令：", time(NULL));
			for (i = 0; i < 32; i++)
				printf("%02x", tmp_psk[i]);
			printf("\n");
			if ((ret = set_psk(net_id, tmp_psk)) < 0) // 设置新口令。
				goto out;
			if ((ret = try_join_in(net_id)) < 0) { // 尝试使用新口令接入WLAN。
				if ((ret = set_psk(net_id, psk)) < 0)
					goto out;
				continue;
			}
			if ((ret = update_initial_psk(psk_path, tmp_psk, psk, &sn)) < 0) // 更新口令文件。
				goto out;
			printf("在时间 %ld 口令更新成功！\n", time(NULL));
		}
		else if (sn + 1 == ap_sn) // 需要但无法更新口令。
			printf("请通过物理认证，进入受控物理环境，而后重新接入WLAN！\n");
		else if (sn + 1 < ap_sn) // 需要但无法更新口令。
			printf("口令已永久失效！\n");
